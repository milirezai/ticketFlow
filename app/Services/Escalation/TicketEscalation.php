...
class TicketEscalation
{
    ...
    {
        return $ticket->assignedTo === null || $user->id !== $ticket->assignedTo->id;
    }

    private function ifNoEligibleExpert(): int
    {
        $support = User::query()
            ->whereHas('roles', fn ($query) => $query->where('name', 'support-specialist'))
            ->inRandomOrder()
            ->first();
        if ($support === null) {
            throw new RuntimeException('No support-specialist available to take an escalated ticket.');
        }
